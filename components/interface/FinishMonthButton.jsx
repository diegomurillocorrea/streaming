...
export function FinishMonthButton({ accountId }) {
    ...
    const handleFinishMonth = async () => {
        ...
        `)
            ...
            if (error) {
                console.error("finish month - load subscriptions error", error);
                return;
            }
            ...
            for (const sub of subs) {
                ...
                if (updateError) {
                    console.error(
                        "finish month - update subscription error",
                        updateError
                    );
                    continue;
                }
                ...
                if (insertError) {
                    console.error("finish month - insert payment error", insertError);
                }
            }
        ...
        <>
            ...
            <AlertDialog open={open} onOpenChange={setOpen}>
                <AlertDialogContent className="bg-emerald-950 border-emerald-800 text-emerald-50">
                    <AlertDialogHeader>
                        <AlertDialogTitle>Finish current month</AlertDialogTitle>
                        <AlertDialogDescription className="text-xs text-emerald-300">
                            This will move all subscriptions of this account to the next
                            period.{" "}
                            <span className="font-semibold">Service Start Date</span> will
                            move to the next month and{" "}
                            <span className="font-semibold">Payment</span> will be reset to{" "}
                            <span className="font-semibold">$0.00</span> for the new month.
                            Previous payments (with their amount and bank) will remain as
                            history.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            ...
                            disabled={loading}
                        >
                            {loading ? "Finishing..." : "Finish month"}
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
