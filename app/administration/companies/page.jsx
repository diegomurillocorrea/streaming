...
export default function CompaniesAdminPage() {
    ...
    const loadCompanies = async () => {
        ...
        if (error) {
            console.error(error);
            setGlobalError(error.message || "Unable to load companies.");
        } else {
            setCompanies(data || []);
        }
    ...
    const handleCreate = async () => {
        ...
        if (error) {
            console.error(error);
            setGlobalError(error.message || "Unable to create company.");
            return;
        }
    ...
    const handleUpdate = async () => {
        ...
        if (error) {
            console.error(error);
            setGlobalError(error.message || "Unable to update company.");
            return;
        }
    ...
    const handleDelete = async () => {
        ...
        if (error) {
            console.error(error);
            setGlobalError(error.message || "Unable to delete company.");
            return;
        }
    ...
    return (
        <main className="max-w-5xl mx-auto space-y-4">
            <header className="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-emerald-300">
                        Manage companies linked to your streaming subscriptions.
                    </p>
                </div>
            ...
            <Card className="border-emerald-800 bg-emerald-900/60">
                ...
                <CardContent>
                    {loading ? (
                        <p className="text-sm text-emerald-300">Loading companies...</p>
                    ) : companies.length === 0 ? (
                        <p className="text-sm text-emerald-300">
