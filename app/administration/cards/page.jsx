...
export default function CardsAdminPage() {
    ...
    const loadReferenceData = async () => {
        ...
        if (netErr) {
            ...
            setNetworksError(netErr.message || "Unable to load payment networks.");
        } else {
        ...
        if (typeErr) {
            ...
            setTypesError(typeErr.message || "Unable to load card types.");
        } else {
            setCardTypes(types || []);
        }
    ...
    const handleCreateCard = async () => {
        if (
            ...
            setCardsError("All fields are required for a card.");
            return;
        }
        ...
        if (!expiryDateForDb) {
            setCardsError("Expiration date must be in MM/YY format.");
            return;
        }
        ...
        if (error) {
            console.error(error);
            setCardsError(error.message || "Unable to create card.");
            return;
        }
    ...
    const handleUpdateCard = async () => {
        ...
        if (error) {
            console.error(error);
            setCardsError(error.message || "Unable to update card.");
            return;
        }
    ...
    const handleDeleteCard = async () => {
        ...
        if (error) {
            console.error(error);
            setCardsError(error.message || "Unable to delete card.");
            return;
        }
    ...
    const handleCreateNetwork = async () => {
        if (!newNetworkName.trim()) {
            setNetworksError("Payment network name is required.");
            return;
        }
        ...
        if (error) {
            console.error(error);
            setNetworksError(error.message || "Unable to create payment network.");
            return;
        }
    ...
    const handleUpdateNetwork = async () => {
        ...
        if (error) {
            console.error(error);
            setNetworksError(error.message || "Unable to update payment network.");
            return;
        }
    ...
    const handleDeleteNetwork = async () => {
        ...
        if (error) {
            console.error(error);
            setNetworksError(error.message || "Unable to delete payment network.");
            return;
        }
    ...
    const handleCreateType = async () => {
        if (!newTypeName.trim()) {
            setTypesError("Card type name is required.");
            return;
        }
        ...
        if (error) {
            console.error(error);
            setTypesError(error.message || "Unable to create card type.");
            return;
        }
    ...
    const handleUpdateType = async () => {
        ...
        if (error) {
            console.error(error);
            setTypesError(error.message || "Unable to update card type.");
            return;
        }
    ...
    const handleDeleteType = async () => {
        ...
        if (error) {
            console.error(error);
            setTypesError(error.message || "Unable to delete card type.");
            return;
        }
    ...
    return (
        <main className="max-w-6xl mx-auto space-y-4">
            <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-sm text-emerald-300">
                        Manage cards and the reference data for payment networks and card
                        types.
                    </p>
                </div>
            ...
            <Card className="border-emerald-800 bg-emerald-900/60">
                <CardHeader>
                    <CardTitle className="text-base">Card list</CardTitle>
                    <CardDescription>
                        All cards stored in the <code>cards</code> table.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {loadingCards ? (
                        <p className="text-sm text-emerald-300">Loading cards...</p>
                    ) : cards.length === 0 ? (
                        <p className="text-sm text-emerald-300">
                            No cards found. Click &quot;New card&quot; to add one.
                        </p>
                    ) : (
                            ...
                            <table className="w-full text-sm border-collapse">
                                <thead>
                                    <tr className="border-b border-emerald-800 text-left text-xs uppercase text-emerald-300">
                                        ...
                                        <th className="py-2 pr-4">Card number</th>
                ...
                <DialogContent className="bg-emerald-950 border-emerald-800 text-white">
                    ...
                    <div className="space-y-3 py-2">
                        <div className="space-y-1">
                            <Label htmlFor="card-owner">Owner name</Label>
                            <Input
                                id="card-owner"
                                type="text"
                        ...
                        <div className="space-y-1">
                            ...
                            >
                                <SelectTrigger className="bg-emerald-900 border-emerald-700">
                                    <SelectValue placeholder="Select a payment network" />
                                </SelectTrigger>
            ...
            <Dialog open={editCardOpen} onOpenChange={setEditCardOpen}>
                <DialogContent className="bg-emerald-950 border-emerald-800 text-white">
                    ...
                    <div className="space-y-3 py-2">
                        <div className="space-y-1">
                            <Label htmlFor="edit-card-owner">Owner name</Label>
                            <Input
                                id="edit-card-owner"
                                type="text"
                                value={formOwnerName}
                        ...
                        <div className="space-y-1">
                            <Label>Edit payment network</Label>
                            <Select
                                value={formNetworkId}
                                onValueChange={setFormNetworkId}
                                disabled={savingCard || loadingRefs}
                        ...
                        <div className="space-y-1">
                            <Label htmlFor="edit-card-expiration">Expiration date</Label>
                            <Input
            ...
            <AlertDialog open={deleteCardOpen} onOpenChange={setDeleteCardOpen}>
                <AlertDialogContent className="bg-emerald-950 border-emerald-800 text-white">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to delete this card for{" "}
            ...
            <Dialog open={manageNetworksOpen} onOpenChange={setManageNetworksOpen}>
                <DialogContent className="bg-emerald-950 border-emerald-800 text-white max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>Payment networks</DialogTitle>
                        <DialogDescription>
                            Create, edit and delete payment networks like Visa, Mastercard,
                            etc.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-2">
                        ...
                        <div className="space-y-1">
                            <Label htmlFor="new-network">New payment network</Label>
                            <div className="flex gap-2">
                                <Input
                                    id="new-network"
                                    type="text"
            ...
            >
                <AlertDialogContent className="bg-emerald-950 border-emerald-800 text-white">
                    <AlertDialogHeader>
                        <AlertDialogTitle>Delete payment network</AlertDialogTitle>
                        <AlertDialogDescription>
                            Are you sure you want to delete{" "}
                            <span className="font-mono text-emerald-200">
                                {networkToDelete?.name}
                            </span>
                            ? This action cannot be undone and may fail if there are cards
                            using this network.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            ...
                            className="bg-red-600 hover:bg-red-700"
                        >
                            {deletingNetwork ? "Deleting..." : "Delete"}
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
            ...
            <Dialog open={manageTypesOpen} onOpenChange={setManageTypesOpen}>
                <DialogContent className="bg-emerald-950 border-emerald-800 text-white max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Create, edit and delete card types such as credit, debit, etc.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-2">
                        ...
                        <div className="space-y-1">
                            <Label htmlFor="new-type">New card type</Label>
                            <div className="flex gap-2">
                                <Input
                                    id="new-type"
                                    type="text"
