...
function buildComputedData(subscriptions, currentMonthKey) {
    const subsWithFlags = subscriptions.map((sub) => {
        ...
        const clientName = `${client.name ?? ""} ${client.lastName ?? ""}`.trim() || "Unnamed client";
    ...
    for (const s of subsWithFlags) {
        for (const p of s.payments || []) {
            ...
            paymentsOfMonth.push({
                ...
                bankName: p.bank_accounts?.bank_name || "Unknown",
            });
        }
    }
...
function drawOverviewReport(doc, monthLabel, m) {
    drawReportHeader(
        doc,
        "Monthly overview",
        "High level summary of your streaming business.",
        monthLabel
    );
    ...
    // Columna izquierda
    doc.text(`Total accounts: ${m.totalAccounts}`, colX1, rowY);
    doc.text(`Active clients: ${m.totalClients}`, colX1, rowY + 18);
    doc.text(`Paid clients: ${m.paidClients}`, colX1, rowY + 36);
    doc.text(`Pending clients: ${m.pendingClients}`, colX1, rowY + 54);

    // Columna derecha
    doc.text(
        `Expected income: $${totalExpected.toFixed(2)}`,
        colX2,
        rowY
    );
    doc.text(
    ...
    doc.text(
        `Collection rate: ${num(m.collectionRate).toFixed(1)} %`,
        colX2,
        rowY + 54
    );
    ...
    doc
        ...
        .fillColor("#6b7280")
        .text(
            "This report is an automatic summary generated from your Streaming Murillo dashboard.",
            margin,
            boxTop + boxHeight + 18,
...
function drawAccountsReport(doc, monthLabel, accountsStats) {
    drawReportHeader(
        ...
        "How each streaming account is doing this month.",
        monthLabel
    );
...
function drawClientsReport(doc, monthLabel, subsWithFlags) {
    drawReportHeader(
        doc,
        "Active clients",
        "All clients with their account and payment status for this month.",
        monthLabel
    );
...
function drawPaymentsReport(doc, monthLabel, paymentsOfMonth) {
    drawReportHeader(
        ...
        "All payments registered during this month.",
        monthLabel
    );
...
function drawPendingReport(doc, monthLabel, subsWithFlags) {
    drawReportHeader(
        doc,
        "Pending payments",
        "Clients who still have pending payments for this month.",
        monthLabel
    );
...
function drawBanksReport(doc, monthLabel, bankStats) {
    drawReportHeader(
        doc,
        "By payment method",
        "How much was collected through each bank/payment method.",
        monthLabel
    );
...
export async function GET(request, context) {
    try {
        ...
        switch (type) {
            ...
            default:
                return NextResponse.json(
                    { error: "Unknown report type", received: type },
                    { status: 400 }
                );
        }
        ...
        return NextResponse.json(
            {
                error: `Error generating report: ${err?.message || "unknown error"
                    }`,
            },
            { status: 500 }
        );
