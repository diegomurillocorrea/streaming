-- Cobro completo al cerrar mes: si period_in_months > 1, cada mes calendario espera
-- round(precio_cuenta / período), coherente con pagos repartidos (ej. $3 / 2 meses).
...
begin
  ...
  loop
    ...
    elsif v_period > 1 then
      v_min_due := round(v_price / v_period::numeric, 2);
      v_complete := coalesce(v_existing_amt, 0) >= v_min_due;
    else
...
comment on function public.close_account_month(bigint) is
  'Cierra el mes: arrears con umbral precio/período si period_in_months>1; avanza service_start_date; inserta pago en 0.';
